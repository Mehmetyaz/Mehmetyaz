...
<template>
    <div class="mb-16 relative" :class="{ 'text-right': isRight }">
        <!-- Technology Header -->
        <div class="flex items-center gap-4 mb-8 group" :class="isRight ? 'flex-row-reverse' : ''">
            <div class="w-12 h-12 rounded-lg bg-[#0A1A1F]/50 p-2 flex items-center justify-center border"
                :class="tech.tag ? 'border-purple-500/30' : 'border-gray-800/30'">
                <div class="w-12 h-12 flex items-center justify-center">
                    <svg class="fill-[#00FF95] w-8 h-8" v-if="tech.name === 'Rust'" viewBox="0 0 128 128">
                        <path
                    ...
                    <svg v-else-if="tech.name === 'Serverless Architecture'" class="fill-[#00FF95] w-8 h-8"
                        ...
                        <g fill="#00FF95">
                            ...
                            <path
                                d="M13.75 0H5a.75.75 0 000 1.5h8.75a.75.75 0 01.75.75v2.5a.75.75 0 01-.75.75H10.5a.75.75 0 000 1.5h3.25A2.25 2.25 0 0016 4.75v-2.5A2.25 2.25 0 0013.75 0zM13 9.75a.75.75 0 01.75-.75A2.25 2.25 0 0116 11.25a.75.75 0 01-1.5 0 .75.75 0 00-.75-.75.75.75 0 01-.75-.75z" />

                        </g>

                    </svg>

                    <svg v-else-if="tech.name === 'Cloud Platforms'" class="fill-[#00FF95] w-8 h-8" viewBox="0 0 16 16"
                        xmlns="http://www.w3.org/2000/svg" fill="none">
                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                        <g id="SVGRepo_iconCarrier">
                            <path fill="#00FF95" fill-rule="evenodd"
                    ...
                    <img v-else :src="tech.icon" class="" />
                </div>
            </div>
            <div>
                <div class="flex items-center gap-2" :class="isRight ? 'flex-row-reverse' : ''">
                    <h4 class="text-xl font-light text-white group-hover:text-[#00FF95] transition-colors">
                        {{ tech.name }}
                    </h4>
                    <span v-if="tech.tag"
                        class="px-2 py-0.5 rounded-full text-xs bg-purple-500/20 text-purple-300 border border-purple-500/30">
                        {{ tech.tag }}
                    </span>
                </div>
                <div class="flex items-center gap-2" :class="isRight ? 'flex-row-reverse' : ''">
                    <span class="text-[#00FF95] text-sm">{{ tech.experience }}</span>
                    <span v-if="tech.note" class="text-purple-300/80 text-sm">
                        {{ tech.note }}
                    </span>
                </div>
            </div>
        ...
        <div class="absolute top-16 bottom-0 w-px bg-gradient-to-b" :class="[
            isRight ? 'right-6' : 'left-6',
            tech.tag ? 'from-purple-500/50 to-transparent' : 'from-gray-800 to-transparent'
        ]">
        </div>

        <!-- Content with Padding -->
        <div class="space-y-8" :class="isRight ? 'pr-12' : 'pl-12'">
            <!-- Points -->
            <div v-for="point in tech.points" :key="point.title">
                <h5 class="text-white text-sm uppercase tracking-wider mb-4">{{ point.title }}</h5>
                <ul class="space-y-3">
                    <li v-for="(item, index) in point.items" :key="index" class="text-gray-400"
                        :class="isRight ? 'text-right' : 'text-left'">
                        <div class="flex items-center gap-2" :class="isRight ? 'flex-row-reverse' : ''">
                            <span class="w-1.5 h-1.5 rounded-full"
                                :class="tech.tag ? 'bg-purple-500/50' : 'bg-[#00FF95]/50'"></span>
                            ...
                            <template v-else>
                                <div class="flex items-center gap-2" :class="isRight ? 'flex-row-reverse' : ''">
                                    <div class="flex items-center gap-2" :class="isRight ? 'flex-row-reverse' : ''">
                                        <span>{{ item.desc }}</span>
                                        <span v-if="item.subdesc" class="text-sm text-gray-500"
                                            :class="isRight ? 'mr-2' : 'ml-2'">
                                            {{ item.subdesc }}
                                        </span>
                                        <span v-if="item.tag"
                                            class="px-2 py-0.5 rounded-full text-xs bg-[#00FF95]/10 text-[#00FF95] border border-[#00FF95]/30"
                                            :class="isRight ? 'mr-2' : 'ml-2'">
                                            {{ item.tag }}
                                        </span>
                                    </div>
